...
void main() {
		...
		mat4 decodedInstanceMatrix = instanceMatrix;

        vec2 decodedOffset = vec2( 17.0 * instanceMatrix[3][0] + 2.5 * position.x, 17.0 * instanceMatrix[3][2] + 2.5 * position.z);
        
		float perlinX = (texture2D( perlinMap, mod(decodedOffset * 0.005 + vec2(time, time), 1.0) ).x - 0.5) * 2.0;
		float perlinY = (texture2D( perlinMap, mod(decodedOffset * 0.007 + vec2(time, time), 1.0) ).x - 0.5) * 2.0;
		float perlinZ = (texture2D( perlinMap, mod(decodedOffset * 0.009 + vec2(time, time), 1.0) ).x - 0.5) * 2.0;

		vec3 windOffset = vec3(perlinX, perlinY, perlinZ);
